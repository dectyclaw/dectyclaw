...
export async function getUserAgents(userId: number) {
  ...
  try {
    ...
    console.error("[Database] Failed to get user agents:", error);
    throw error;
  }
}

export async function getAgentByDNA(dna: string) {
  const db = await getDb();
  if (!db) {
  ...
  try {
    const result = await db.select().from(agents).where(eq(agents.dna, dna)).limit(1);
    return result.length > 0 ? result[0] : undefined;
  } catch (error) {
    console.error("[Database] Failed to get agent by DNA:", error);
    throw error;
  }
}
